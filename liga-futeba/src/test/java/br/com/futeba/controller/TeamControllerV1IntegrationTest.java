...
public class TeamControllerV1IntegrationTest {
	...
	@Before
	public void setUp() {
		this.originais = new Team(1L, TestUtil.TEAM_ORIGINAIS, false,
				TestUtil.TEAM_RESPONSABLE,
				TestUtil.TEAM_RESPONSABLE_PHONE_NUMBER);
		this.veneza = new Team(2L, TestUtil.TEAM_VENEZA, false,
				TestUtil.TEAM_RESPONSABLE,
				TestUtil.TEAM_RESPONSABLE_PHONE_NUMBER);
		this.sanRemo = new Team(3L, TestUtil.TEAM_SAN_REMO, false,
				TestUtil.TEAM_RESPONSABLE,
				TestUtil.TEAM_RESPONSABLE_PHONE_NUMBER);
	...
	public void givenTeams_whenDeleteTeamById_thenStatus200() throws Exception {
		...
		mvc.perform(delete("/api/v1/teams/1")).andDo(print())
				.andExpect(status().isOk());
